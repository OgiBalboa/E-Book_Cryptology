...
class db:
	def __init__(self,):
		...
		offpwscrt = os.path.join(self.documents_path, "offpwscrt.bin")
		try:
			with open(offpwscrt, "r") as okey:
		...
		self.db = db_
		try: self.unzip_key = self.db.reference("App").get()["unzip_key"]
		except: pass
	...
	def upload_book(self,name,path):
		...
		return True
	def download_book(self,name,path):
		blob = self.storage.blob("books/" + name + ".zip")
		blob._chunk_size = 	1024*1024
		blob.download_to_filename(path)
		self.main.unzip(path)
		self.main.check_library()
	...
	def retrieve_info(self):
		try:
			...
			for book in self.books:
				if day_difference(book[3],self.main.login_time) < 0:
					print("ahaa")
...
def day_difference (date1,date2):
	'''
	:param date1: must be login time
	:param date2: must be book date
	:return: day difference (int)
	'''
	try: date_1 = date1.split(".")
	except :
		date_1 =[date1.day,date1.month,date1.year]
	date_2 = date2.split(".")
	date_1 = datetime.date(int(date_1[2]),int(date_1[1]),int(date_1[0]))
	date_2 = datetime.date(int(date_2[2]),int(date_2[1]),int(date_2[0]))
